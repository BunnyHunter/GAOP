...
impl<T: Attribute> ChangeAttribute<T> for CharacterCommonData {
    fn change_attribute(&self, attribute: &mut T) {
        attribute.set_value_by(AttributeName::ATKBase, "Character Base Attack", self.base_atk);
        attribute.set_value_by(AttributeName::DEFBase, "Character Base Defense", self.base_def);
        attribute.set_value_by(AttributeName::HPBase, "Character Base HP", self.base_hp);
