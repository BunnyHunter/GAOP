...
impl<T: Attribute> WeaponEffect<T> for TheAlleyFlashEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.set_value_by(AttributeName::BonusBase, "The Alley Flash Passive", value * self.rate);
    }
}
...
impl WeaponTrait for TheAlleyFlash {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Itinerant Hero: Increases DMG dealt by the character equipping this weapon by 12/15/18/21/24%. Taking DMG disables this effect for 5s."),
        #[cfg(not(target_family = "wasm"))]
        chs: "The Alley Flash"
    };
