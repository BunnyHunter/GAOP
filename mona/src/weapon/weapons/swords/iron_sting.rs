...
impl<T: Attribute> WeaponEffect<T> for IronStingEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.set_value_by(AttributeName::BonusBase, "Iron Sting Passive Equivalent", value);
    }
}
...
impl WeaponTrait for IronSting {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Infused Stinger: Dealing Elemental DMG increases all DMG by 6/7.5/9/10.5/12% for 6s. Max 2 stacks. Can only occur once every 1s."),
        #[cfg(not(target_family = "wasm"))]
        chs: "Iron Sting"
    };
