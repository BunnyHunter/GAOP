...
impl<T: Attribute> WeaponEffect<T> for MappaMareEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.add_elemental_bonus("Mappa Mare Passive Equivalent", value);
    }
}
...
impl WeaponTrait for MappaMare {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Infusion Scroll: Triggering an Elemental reaction grants a 8/10/12/14/16% Elemental DMG Bonus for 10s. Max 2 stacks."),
        #[cfg(not(target_family = "wasm"))]
        chs: "Mappa Mare"
    };
