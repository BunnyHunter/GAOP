...
impl<T: Attribute> WeaponEffect<T> for AlleyHunterEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.set_value_by(AttributeName::BonusBase, "Alley Hunter Passive Equivalent", value);
    }
}
...
impl WeaponTrait for AlleyHunter {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Oppidan Ambush: While the character equipped with this weapon is in the party but not on the field, their DMG increases by 2/2.5/3/3.5/4% every second up to a max of 20/25/30/35/40%. When the character is on the field for more than 4s, the afromentioned DMG bugg decreases by 4/5/6/7/8% per second until it reaches 0%."),
        #[cfg(not(target_family = "wasm"))]
        chs: "AlleyHunter",
    };
