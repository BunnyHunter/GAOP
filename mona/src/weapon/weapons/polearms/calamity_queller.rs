...
impl<T: Attribute> WeaponEffect<T> for CalamityQuellerEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.add_elemental_bonus("Calamity Queller Passive", elemental_bonus);

        let atk_bonus = (refine * 0.008 + 0.024) * self.stack * (1.0 + self.backend_rate);
        attribute.add_atk_percentage("Calamity Queller Passive Equivalent", atk_bonus);
    }
}
...
impl WeaponTrait for CalamityQueller {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Extinguishing Precept: Gain 12/15/18/21/24% All Elemental DMG Bonus. Obtain Consummation for 20s after using an Elemental Skill, causing ATK to increase by 3.2/4/4.8/5.6/6.4% per second. This ATK increase has a maximum of 6 stacks. When the character equipped with this weapon is not on the field, Consummation ATK increase is doubled."),
        #[cfg(not(target_family = "wasm"))]
        chs: "Calamity Queller"
    };
