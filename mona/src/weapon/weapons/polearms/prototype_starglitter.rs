...
impl<T: Attribute> WeaponEffect<T> for PrototypeStarglitterEffect {
    fn apply(&self, data: &WeaponCommonData, attribute: &mut T) {
        ...
        attribute.set_value_by(AttributeName::BonusNormalAttack, "Prototype Starglitter", value);
        attribute.set_value_by(AttributeName::BonusChargedAttack, "Prototype Starglitter", value);
    }
}
...
impl WeaponTrait for PrototypeStarglitter {
    const META_DATA: WeaponStaticData = WeaponStaticData {
        ...
        effect: Some("Magic Eater: After using an Elemental Skill, increases Normal and Charged Attack DMG by 8/10/12/14/16% for 12s. Max 2 stacks."),
        #[cfg(not(target_family = "wasm"))]
