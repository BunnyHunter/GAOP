        ...
        <div v-else-if="!calculating && !error.isError">
            ...
            <el-row :gutter="16">
                <el-col :span="18">
                    ...
                    <div>
                        <el-button size="small" @click="handleSaveAsNewKumi">存为圣遗物Set</el-button>
                        <el-button size="small" @click="handleToggleAll">{{ isAllDisabled ? "Enabled" : "Disable" }}全部</el-button>
                    </div>

                    <h3 class="title">最大值</h3>
                    <el-alert
                        title="不同目标函数的最大值不可相互比较；Damage类型的最大值也并不是最终期望伤害，因此仅供参考"
                        type="warning"
                        :closable="false"
    ...
    methods: {
        ...
        doCompute(artifacts, veryBigConfigObject) {
            ...
            let promise = compute(
                ...
                this.error = {
                    isError: true,
                    reason: "An error occurred during the calculation: " + reason
                };
                loading.close();
                this.calculating = false;
            });
